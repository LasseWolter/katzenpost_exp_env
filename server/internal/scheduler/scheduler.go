...
func (sch *scheduler) worker() {
	...
	// The following variables are used for printing out the message queue length on regular intervals
	//queuePollInterval := sch.glue.Config().Debug.QueuePollInterval
	//queueLogFolder := sch.glue.Config().Debug.QueueLogDir
	queuePollInterval := sch.glue.CliConfig().Experiment.QueuePollInterval
	queueLogFolder := sch.glue.CliConfig().Experiment.QueueLogDir
	serverName := sch.glue.Config().Server.Identifier
	path := filepath.Join(".", "queue_outputs", queueLogFolder)
	if _, err := os.Stat(path); os.IsNotExist(err) {
		_ = os.MkdirAll(path, 0777)
	}
	qlFileName := fmt.Sprintf("%v/ql_%v", path, serverName)
	f, err := os.Create(qlFileName)
	if err != nil {
		sch.log.Errorf("Error creating file for storing queue lengths: %s", err)
	} else {
		sch.log.Info("Successfully created log file for queue lengths in %v", qlFileName)
	}
	// Setup ticker and start (async) for polling server for queue length on regular intervals
	var ticker time.Ticker
	go func() {
		sch.log.Debugf("QueueLength Ticker started - Queue Length will be polled every %dms", queuePollInterval)
		ticker = *time.NewTicker(time.Duration(queuePollInterval) * time.Millisecond)
		for range ticker.C {
			sch.log.Debugf("Current Queue length: %v", sch.q.Len())
			queueStr := fmt.Sprintf("%v,%v\n", time.Now().Format(time.StampMilli), sch.q.Len())
			sch.glue.PKI()
			_, err := f.WriteString(queueStr)
			if err != nil {
				sch.log.Errorf("Error writing queue length to file: %s", err)
			}
		}
	}()
	defer f.Close()
	...
	for {
		...
		// Asynchronous function to print out the queueLength in regular intervals
		go func() {
